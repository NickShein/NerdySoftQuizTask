...
export class PlayComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(
      combineLatest([this.route.paramMap, this.store.select(getCategorizedQuizz)]).subscribe(([params, categorized]) => {
  ...
  countRightAnswersAndPoints(answer:string): void{
    let correctAnswer = this.categorizedArrays[this.currentQuestionNumber].correct_answer;
    let point = 0;
    this.timeEnd = new Date();
    if(answer==correctAnswer ){
      let point = 1;
      if(this.categorizedArrays[this.currentQuestionNumber].incorrect_answers.length>1){
        point = 3;
      }
      this.answers.push({answer:answer, correct_answer: correctAnswer, isCorrect:true, secondsSpent: this.getDifferenceInMiliseconds(this.timeEnd, this.timeStart), points: point});
    }
    else{
      this.answers.push({answer:answer, correct_answer: correctAnswer, isCorrect:false, secondsSpent: this.getDifferenceInMiliseconds(this.timeEnd, this.timeStart), points: point});
    }
    this.timeStart = new Date();
  }
  ...
  calculatePoints(array: Answer[]){
    return array.reduce((totalScore, answer) => {
      if (answer.isCorrect) {
        ...
        if (answer.correct_answer === "True" || answer.correct_answer === "False") {
          return totalScore + 1;
        } else {
